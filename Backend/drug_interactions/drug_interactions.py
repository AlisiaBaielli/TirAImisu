...
class InteractionReport(BaseModel):
    interaction_found: bool = Field(
        ..., description="Set to true if an interaction is found, false otherwise."
    )
    severity: Optional[str] = Field(
        None,
        description="The severity of the interaction (e.g., 'Mild', 'Moderate', 'Severe') if found.",
    )
    description: Optional[str] = Field(
        None, description="A concise, one-sentence summary of the interaction if found."
    )
    extended_description: Optional[str] = Field(
        None,
        description="A detailed explanation of the interaction's mechanism, effects, and management if found.",
    )
...
def check_interaction_with_llm(
    ...
    try:
        response = client.chat.completions.create(
            ...
            tool_choice={
                "type": "function",
                "function": {"name": "report_interaction"},
            },
        )
...
def synthesize_reports(
    reports: List[InteractionReport], drug_a: str, drug_b: str
) -> Optional[InteractionReport]:
    """
...
def main():
    ...
    new_drug = input(
        "Enter the NEW medication to check:\n(e.g., Fluoxetine, Phenelzine)\n> "
    ).strip()
    ...
    if not existing_drug_list:
        print(
            "No existing medications to check against. New drug can be added safely (in this context)."
        )
    ...
    for existing_drug in existing_drug_list:
        ...
        if text_new_drug:
            report_a = check_interaction_with_llm(
                text_new_drug, new_drug, existing_drug
            )
        ...
        if text_existing_drug:
            report_b = check_interaction_with_llm(
                text_existing_drug, existing_drug, new_drug
            )
            if report_b and report_b.interaction_found:
                # Avoid adding a near-duplicate report
                if not any(
                    r.description == report_b.description for r in reports_for_this_pair
                ):
                    reports_for_this_pair.append(report_b)

        # Synthesize the findings for this one pair
        if reports_for_this_pair:
            final_pair_report = synthesize_reports(
                reports_for_this_pair, new_drug, existing_drug
            )
    ...
    if all_final_reports:
        print(
            f"🚨 Found {len(all_final_reports)} potential interaction(s) between '{new_drug}' and the existing medication list."
        )
        ...
        print(
            f"✅ No specific interactions were found between '{new_drug}' and the existing medications."
        )
